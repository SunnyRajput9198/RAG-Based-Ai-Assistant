...
async def chat(query: Query):
    ...
    prompt = f'''Sigma Web Development course ke video chunks:
{new_df[["title", "number", "start", "end", "text"]].to_json(orient="records")}
Question: "{query.message}"
Answer with video number and timestamp.'''

    response = agent.invoke({"messages": [{"role": "user", "content": prompt}]}, config)
    
    sources = [{"videoId": int(row["number"]), "videoTitle": row["title"],
                 "timestamp": f"{int(row['start'])//60}:{int(row['start'])%60:02d}"}
                for _, row in new_df.iterrows()]

    return {"content": response["messages"][-1].content, "sources": sources}